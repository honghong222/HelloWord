	...
	<div class="row">
		<div class="col-sm-4">
			<select name="searchCondition" class="form-control">
				...
				<option value="TW"
					${searchCondition ne  null && searchCondition =='TW' ? 'selected' : ''}>제목
					& 작성자</option>
			</select>
		</div>
		<div class="col-sm-6">
...
<nav aria-label="...">
	<ul class="pagination">
		<!-- 이전 10개의 목록이 존재하면 Previous활성화 -->
		<c:choose>
			<c:when test="${paging.prev }">
				<li class="page-item"><a class="page-link"
					href="boardList.do?keyword=${keyword ne null ? keyword : '' }&searchCondition=${!empty searchCondition ? searchCondition : '' }&page=${paging.startPage -1}">previous</a>
				</li>
			</c:when>
		...
		<!--  StartPage와 endPage의 값에 해당하는 링크를 반복생성-->

		<!--//출력하는 페이지와 전체 페이지가 없으면 링크생략-->
		...
		<c:forEach var="p" begin="${paging.startPage }"
			...
			<c:choose>
				...
				<c:otherwise>
					<li class="page-item"><a class="page-link"
						href="boardList.do?keyword=${keyword eq null ? '' : keyword }&searchCondition=${searchCondition eq null ? '': searchCondition }&page=${p}">${p}</a></li>
				</c:otherwise>
			</c:choose>
		</c:forEach>
		...
		<c:choose>
			<c:when test="${paging.next }">
				<li class="page-item"><a class="page-link"
					href="boardList.do?keyword=${keyword eq null ? '' : keyword  }&searchCondition=${searchCondition eq null ?'': searchCondition   }&page=${paging.endPage +1}">Next</a>
				</li>
				</c:when>
					<c:otherwise>
